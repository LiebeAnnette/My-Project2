  ...
  const handleGenreClick = async (genre: Genre) => {
    ...
    try {
      ...
      const artistRes = await fetch(
        `${BASE_URL}/api/genre/${genre.id}/artists`
      );
      ...
      const trackRes = await fetch(`${BASE_URL}/api/artist/${artist.id}/top`);
      ...
      const randomTrack =
        trackData.data[Math.floor(Math.random() * trackData.data.length)];
